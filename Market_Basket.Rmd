...
style(weekday_plot, hoverinfo = "y")
...
ts_df <- as.data.frame.Date(seq(min(daily_sales$Date), max(daily_sales$Date), by = "day"))
colnames(ts_df) <- "Date"
...
as_ts <- ts(ts_df$Daily_Value, start = min(ts_df$Date), end =  max(ts_df$Date), frequency = 53 )
